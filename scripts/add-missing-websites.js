...
function extractActualUrl(website) {
  ...
  // Extract potential domain from name
  ...
  for (const tld of tlds) {
    ...
    // Return first reasonable guess (we'll let users verify later)
    return potentialUrl;
  }
...
// Main function to add missing websites
function addMissingWebsites() {
  const missingPath = path.join(__dirname, 'unique-missing-websites.json');
  const csvPath = path.join(__dirname, '..', 'data', 'websites.csv');
  
  console.log('Loading missing websites...');
  const missingWebsites = JSON.parse(fs.readFileSync(missingPath, 'utf-8'));
  console.log(`Found ${missingWebsites.length} missing websites\n`);
  ...
  const existingWebsites = parsed.data.slice(1); // Skip header
  
  console.log(`Current database has ${existingWebsites.length} websites\n`);
  
  // Create a set of existing URLs and names for deduplication
  ...
  console.log('Processing missing websites...');
  const newWebsites = [];
  let skipped = 0;
  ...
  for (const website of missingWebsites) {
    ...
    const normalizedUrl = actualUrl.toLowerCase().replace(/^https?:\/\//, '').replace(/\/$/, '');
    ...
    // Create CSV row: name, url, category, description, featured, hidden
    newWebsites.push([
      name,
      actualUrl,
      category,
      description,
      '', // featured (empty = false)
      ''  // hidden (empty = false)
    ]);
  }
  
  console.log(`\nProcessed ${missingWebsites.length} websites:`);
