...
import { liveCardSites } from "@/lib/archive";
...
export default async function Home() {
  ...
  const curated: CardSite[] = [];
  const seen = new Set<string>();
  for (const site of [...canonicalCards(), ...live, ...SITES]) {
    if (seen.has(site.slug)) continue;
    seen.add(site.slug);
    curated.push(site);
    if (curated.length >= 6) break;
  }
  const heroCards = curated.length >= 3 ? curated.slice(0, 3) : SITES.slice(0, 3);
