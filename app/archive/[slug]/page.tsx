...
import {
  getCatalogWebsite,
  relatedCardsForWebsite,
  websiteToRecord,
} from "@/lib/archive";
import CatalogSitePage from "@/components/CatalogSitePage";
...
export async function generateMetadata({
  ...
  const live = await getCatalogWebsite(slug);
  if (!live) return { title: "Record not found" };
  return {
    title: `${live.name} — Website Design`,
    description: live.description,
    openGraph: {
      title: live.name,
      description: live.description,
      images: [live.screenshotUrl],
      type: "website",
      url: `https://www.allwebsites.design/archive/${slug}`,
    },
...
export default async function SitePage({
  ...
  const live = await getCatalogWebsite(slug);
  if (!live) notFound();
  const similar = await relatedCardsForWebsite(live, 3);
  return <CatalogSitePage site={websiteToRecord(live)} similar={similar} />;
}
